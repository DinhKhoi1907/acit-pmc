    ...
    <div class="mail-container">
        ...
        <div class="mail-body">
            <div class="mail-content-inform" style="background: no-repeat url({{config('config_all.config_all_url')}}img/mail/logo-mikotech.png) center;background-size: contain;">
                <p class="mail-title">{{($setting) ? $setting['tenvi'] : 'Chúng tôi'}} xin thông báo!<br> Hệ thống phát hiện tài khoản của bạn đang có người muốn thay đổi thông tin email, nếu là bạn xin vui lòng nhấp <a href="{{route('account.verification',['data'=>md5($user['email_new'].'|'.$user['email_verify_time'])])}}" target="_blank"> tại đây </a>để xác minh danh tính và cập nhật email.</p>
            </div>
        </div>
