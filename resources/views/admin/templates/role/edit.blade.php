        ...
        <div class="card card-permission card-primary card-outline text-sm parent-select">
            <div class="card-header">
                <div class="card-tools-dev">
                    <div class="icheck-primary d-inline dev-check">
                        ...
                        <label for="selectAll-static-photo">Quản lý danh mục</label>
                    </div>
                </div>
            ...
            <div class="card-body">
                @foreach(config('category') as $key => $value)
                    {{-- hidden --}}
                    @if(Helper::CheckPermissionParent($permissions_parent,'category_man_'.$key))<input type="hidden" class="custom-control-input" name="dataPermission[]" value="category_man_{{$key}}">@endif

                    @if(Helper::CheckPermissionParent($permissions_parent,'category_add_'.$key))<input type="hidden" class="custom-control-input" name="dataPermission[]" value="category_add_{{$key}}">@endif

                    @if(Helper::CheckPermissionParent($permissions_parent,'category_edit_'.$key))<input type="hidden" class="custom-control-input" name="dataPermission[]" value="category_edit_{{$key}}">@endif

                    @if(Helper::CheckPermissionParent($permissions_parent,'category_delete_'.$key))<input type="hidden" class="custom-control-input" name="dataPermission[]" value="category_delete_{{$key}}">@endif

                    <div class="form-group row">
                        <label class="d-inline-block align-middle mb-2 mr-2 text-md col-md-3" for="quyen-man-photo-{{$key}}">{{$value['title_main']}}:</label>
                        <div class="col-md-7">
                            @if(Helper::CheckPermissionParent($permissions_parent,'category_man_'.$key))
                            <div class="icheck-primary d-inline dev-check align-middle mb-2 mr-4 text-md">
                                <input type="checkbox" class="select-checkbox" name="dataQuyen[]" id="quyen-man-category-view-{{$key}}" value="category_man_{{$key}}" {{ (isset($permissions) && in_array('category_man_'.$key,$permissions))?'checked':'' }}>
                                <label for="quyen-man-category-view-{{$key}}" class="font-weight-normal">Xem</label>
                            </div>
                            @endif

                            @if(Helper::CheckPermissionParent($permissions_parent,'category_add_'.$key))
                            <div class="icheck-primary d-inline dev-check align-middle mb-2 mr-4 text-md">
                                <input type="checkbox" class="select-checkbox" name="dataQuyen[]" id="quyen-man-category-add-{{$key}}" value="category_add_{{$key}}" {{ (isset($permissions) && in_array('category_add_'.$key,$permissions))?'checked':'' }}>
                                <label for="quyen-man-category-add-{{$key}}" class="font-weight-normal">Thêm mới</label>
                            </div>
                            @endif

                            @if(Helper::CheckPermissionParent($permissions_parent,'category_edit_'.$key))
                            <div class="icheck-primary d-inline dev-check align-middle mb-2 mr-4 text-md">
                                <input type="checkbox" class="select-checkbox" name="dataQuyen[]" id="quyen-man-category-edit-{{$key}}" value="category_edit_{{$key}}" {{ (isset($permissions) && in_array('category_edit_'.$key,$permissions))?'checked':'' }}>
                                <label for="quyen-man-category-edit-{{$key}}" class="font-weight-normal">Chỉnh sửa</label>
                            </div>
                            @endif

                            @if(Helper::CheckPermissionParent($permissions_parent,'category_delete_'.$key))
                            <div class="icheck-primary d-inline dev-check align-middle mb-2 text-md">
                                <input type="checkbox" class="select-checkbox" name="dataQuyen[]" id="quyen-man-category-delete-{{$key}}" value="category_delete_{{$key}}" {{ (isset($permissions) && in_array('category_delete_'.$key,$permissions))?'checked':'' }}>
                                <label for="quyen-man-category-delete-{{$key}}" class="font-weight-normal">Xóa</label>
                            </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="card-header">
                <div class="card-tools-dev">
                    <div class="icheck-primary d-inline dev-check">
                        ...
                        <label for="selectAll-tags">Quản lý tags</label>
                    </div>
                </div>
                <div class="card-tools">
                	<button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
