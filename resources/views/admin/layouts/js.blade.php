...
<script type="text/javascript">
	...
	$(document).ready(function(){
        ...
        photoZone("#photo-other1","#file-other1","#photoUpload-other1 img");
        photoZone("#photo-other2","#file-other2","#photoUpload-other2 img");
        photoZone("#photo-other3","#file-other3","#photoUpload-other3 img");
	...
	CKEDITOR.editorConfig = function( config ) {
		...
		/* All Plugins */
		config.extraPlugins = 'texttransform,copyformatting,html5video,html5audio,flash,youtube,wordcount,tableresize,widget,lineutils,clipboard,dialog,dialogui,widgetselection,lineheight,video,videodetector,image';
	...
	function deleteFiler(string){
		...
		$('.my-jFiler-item-'+id).addClass('d-none');
	...
	function onSearch(obj,url){
		if(url==''){
			...
			url = filter_category(url);
			if(keyword){
				if(obj=='keyword_noidung'){
					url += "&keyword_noidung="+encodeURI(keyword);
				}else{
					url += "&keyword="+encodeURI(keyword);
				}				
			}
  	...
  	$(document).ready(function(){
		...
		$("#filer-gallery").filer({
            ...
            afterShow: function(){
            	...
            	console.log('filer-gallery | afterShow');
            },
            uploadFile: {
				...
				onComplete: function(){
					console.log('filer-gallery | complete');
					refreshFiler(e_filter_main);
					...
					{					
						//if(!$('.form-group-gallery-main').exists()){refreshFilerIfEmpty(e_filter_main);}
						$('.form-group-gallery-main').remove();
