@php
    ...
    $max_level = (isset($config[$type]['max_level'])) ? $config[$type]['max_level'] : 0;
@endphp
            ...
            <div class="px-0 mb-0 form-group col-sm-12">
                <select class="form-control" name="data[level]" id="category-multy-level">
                    @if($prefix=='category')
                        ...
                        <option value="{{($i)}}" {{(isset($rowItem) && $rowItem['level']==$i) ? 'selected' : ''}}>{{__('Cấp')}} {{($i+1)}}</option>
                        @endfor
                        {{-- <option value="0" {{(isset($rowItem) && $rowItem['level']==0) ? 'selected' : ''}}>{{__('Cấp')}} 1</option>
                        <option value="1" {{(isset($rowItem) && $rowItem['level']==1) ? 'selected' : ''}}>{{__('Cấp')}} 2</option>
                        <option value="2" {{(isset($rowItem) && $rowItem['level']==2) ? 'selected' : ''}}>{{__('Cấp')}} 3</option> --}}
                    @else
...
@push('js')
    ...
    <script>
        ...
        $('body').on('change', '.multy-checkbox', function(){
            ...
            list_name = (name_selected.length>0) ? name_selected.toString().replace(",", ", ") : 'Select a category';
            ...
            if(list_ids!='' && level_child<=max_level){
                $.ajax({
                    ...
                    success: function(result){
                        ...
                        LoadProperty(id,list_ids,level-1);
                    }
                });
            }
        });
        ...
        function LoadProperty(id_product,list_ids,level){
            $.ajax({
                url: "{{ route('admin.ajax.loadProperty') }}",
                type: 'GET',
                dataType: 'html',
                data: {id_product:id_product,id:list_ids, level:level},
                success: function(result){
                    $('#list_property').html(result);
                }
            });
        }
    </script>
