    ...
    <div class="text-sm card card-primary card-outline">
        ...
        <div class="card-body form-group-category">
	        ...
	        @if(count($productOption_allXoatam)>0 && count($all_option)>0 && $row_product)
	            <p class="dev-option-error text-success col-xl-12"><i class="fas fa-check-circle"></i> Vào mục <a href="{{route('admin.productOption.show',['man',$row_product['type'],'id_product'=>$row_product['id']])}}" target="_blank">Quản lý phiên bản</a> để thêm các phiên bản con</p>
	        @endif
		    ...
		    @foreach($options_group as $g_opt=>$g_option)
		    	...
		    	<div class="dev-options-group">
		    		...
		    		<div class="mb-3 dev-options-group-info">
		    			@if($info_color)
		    			<p class="dev-options-group-color"><span style="background:#{{$info_color['mau']}}" class="mr-2 dev-options-info-bg"></span> {{$info_color['tenvi']}}</p>
		    			{{-- <div class="p-0 mb-4 form-group col-md-4 col-sm-12">
							<label for="masp_color_{{$g_opt}}" class="inp">
								<input type="text" class="form-control for-seo" name="dataColor[{{$g_opt}}]" id="masp_color_{{$g_opt}}" placeholder="&nbsp;" value="{{(isset($masp_colors[$g_opt])) ? $masp_colors[$g_opt] : ''}}">
		    		...
		    		@foreach($options as $op=>$option)
		            	@if($option['xoatam']==0)
		                    <div class="dev-options-item dev-options-item-{{$op}}" data-op="{{$op}}">
		                        <div class="dev-hidden-info">
		                            <input type="hidden" name="dataOption[{{$g_opt}}-{{$op}}][id]" value="{{$option['id']}}" id="hidden_idoption_{{$g_opt}}_{{$op}}">
		                            <input type="hidden" name="dataOption[{{$g_opt}}-{{$op}}][id_mau]" value="{{$option['id_mau']}}" >
		                            <input type="hidden" name="dataOption[{{$g_opt}}-{{$op}}][id_size]" value="{{$option['id_size']}}" >
		                            <input type="hidden" name="dataOption[{{$g_opt}}-{{$op}}][tenkhongdauvi]" value="{{$option['tenkhongdauvi']}}" >
		                            <input type="hidden" name="dataOption[{{$g_opt}}-{{$op}}][tenvi]" value="{{$option['tenvi']}}" >
		                            <input type="hidden" name="dataOption[{{$g_opt}}-{{$op}}][tenen]" value="{{$option['tenvi']}}" >
		                        </div>

		                        <div class="dev-options-info">
		                            <div id="photoUpload-preview-{{$op}}" class="dev-options-img">
		                                <img src="{{config('config_upload.UPLOAD_PRODUCT').$option['photo']}}" onerror=src="{{asset("img/noimage.png")}}" alt="image">
		                            </div>
		                            <div class="dev-options-main">
		                                <p class="dev-options-name">{{$option['tenvi']}}</p>
		                                @if($option['id'])
		                                	<div class="dev-options-main-price">
		                                		<div class="dev-options-giachange" data-id="{{$option['id']}}"><span>Giá bán</span><input type="text" class="dev-options-gia format-price gia_ban" placeholder="Giá bán" value="{{$option['gia']}}"><p class="btn-giachange" data-type="gia"><i class="fas fa-check"></i><i class="fas fa-ban"></i>Thay đổi</p></div>
		                                		<div class="dev-options-giachange" data-id="{{$option['id']}}"><span>Giá mới</span><input type="text" class="dev-options-giamoi format-price gia_moi" placeholder="Giá mới" value="{{$option['giamoi']}}"><p class="btn-giachange" data-type="giamoi"><i class="fas fa-check"></i><i class="fas fa-ban"></i>Thay đổi</p></div>
												@if(config('config_all.order.soluong'))
												<div class="dev-options-giachange" style="width:100%" data-id="{{$option['id']}}"><span>Số lượng</span><input type="text" class="dev-options-soluong format-price soluong" placeholder="Số lượng" value="{{$option['soluong_website']}}"><p class="btn-giachange" data-type="soluong_website"><i class="fas fa-check"></i><i class="fas fa-ban"></i>Thay đổi</p></div>
												@endif
		                                	</div>
		                                	<a class="text-info dev-edit dev-options-edit" href="{{route('admin.productOption.edit',['man',$type,$option['id']])}}" target="_blank"><i class="pl-1 mr-1 far fa-edit"></i></a>
		                                	<a class="mr-3 text-info text-danger dev-edit delete-option dev-options-delete" data-op="{{$op}}" data-id="{{$option['id']}}" ><i class="pl-1 mr-1 far fa-trash-alt"></i></a>
		                                @else
		                                	<a class="mr-3 text-info dev-edit" data-toggle="modal" data-target="#modal_option_{{$g_opt}}_{{$op}}"><i class="mr-1 far fa-edit"></i>Chỉnh sửa phiên bản</a>
		                                ...
		                                <div class="modal fade" id="modal_option_{{$g_opt}}_{{$op}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
		                                    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
		                                        <div class="modal-content">
		                                            <div class="modal-header">
		                                                <h5 class="modal-title" id="exampleModalLongTitle">{{$option['tenvi']}}</h5>
		                                            ...
		                                            <div class="modal-body">
		                                                ...
		                                                <div class="dev-options-image d-none">
		                                                    <div class="photoUpload-zone">
		                                                        <label class="photoUpload-file" id="photo-zone-{{$op}}" for="file-zone-{{$op}}">
		                                                            <input type="file" name="file-{{$op}}-{{$op}}" id="file-zone-{{$op}}">
		                                                ...
		                                                <div class="row">
		                                                    <!--ID Hidden-->
		                                                    <input type="hidden" id="id_option" name="dataOption[{{$g_opt}}-{{$op}}][masp]" value="{{$op}}" required>
		                                                    <input type="hidden" name="dataOption[{{$g_opt}}-{{$op}}][xoatam]" value="{{$option['xoatam']}}" id="hidden_xoatam_option_{{$op}}" required>
		                                                    <!--Mã SP-->
		                                                    <div class="form-group col-md-6">
		                                                        <label class="d-block" for="masp">Mã sản phẩm:</label>
		                                                        <input type="text" class="form-control masp_option_danger" name="dataOption[{{$g_opt}}-{{$op}}][masp]" id="masp_option_{{$g_opt}}_{{$op}}" placeholder="Mã sản phẩm" value="{{(($option['masp']) ? $option['masp'] : $parent_masp.'-'.$g_opt.'-'.($op+1))}}" data-op="{{$op}}" data-group="{{$g_opt}}" required>
		                                                        <p class="mt-2 mb-0 alert-masp text-danger d-none" id="alert-masp_option_{{$g_opt}}_{{$op}}-danger">
		                                                            <i class="mr-1 fas fa-exclamation-triangle"></i>
		                                                            <span>Mã SP đã tồn tại.</span>
		                                                        </p>
		                                                        <p class="mt-2 mb-0 alert-masp text-success d-none" id="alert-masp_option_{{$g_opt}}_{{$op}}-success">
		                                                    ...
		                                                    <div class="form-group col-md-6 d-none">
		                                						...
		                                						<div class="input-group">
		                                							<input type="text" class="form-control format-price dai_option" name="dataOption[{{$g_opt}}-{{$op}}][dai]" id="dai_option_{{$g_opt}}_{{$op}}" placeholder="Dài" value="{{$option['dai']}}" data-position="{{$op}}" data-groupPos="{{$g_opt}}">
		                                							<div class="input-group-append">
		                                								<div class="input-group-text"><strong>x</strong></div>
		                                							</div>
		                                							<input type="text" class="form-control format-price rong_option" name="dataOption[{{$g_opt}}-{{$op}}][rong]" id="rong_option_{{$g_opt}}_{{$op}}" placeholder="Rộng" value="{{$option['rong']}}" data-position="{{$op}}" data-groupPos="{{$g_opt}}">
		                                							<div class="input-group-append">
		                                								<div class="input-group-text"><strong>x</strong></div>
		                                							</div>
		                                							<input type="text" class="form-control format-price cao_option" name="dataOption[{{$g_opt}}-{{$op}}][cao]" id="cao_option_{{$g_opt}}_{{$op}}" placeholder="Cao" value="{{$option['cao']}}" data-position="{{$op}}" data-groupPos="{{$g_opt}}">
		                                							<div class="input-group-append">
		                                                    ...
		                                                    <div class="form-group col-md-6 d-none">
		                                						...
		                                						<div class="input-group">
		                                							<input type="text" class="form-control format-price khoiluong_option" name="dataOption[{{$g_opt}}-{{$op}}][khoiluong]" id="khoiluong_option_{{$g_opt}}_{{$op}}" placeholder="Khối lượng" value="{{$option['khoiluong']}}">
		                                							<div class="input-group-append">
		                                                    ...
		                                                    <div class="form-group col-md-6">
		                                						...
		                                						<div class="input-group">
		                                							<input type="text" class="form-control format-price soluong_option" name="dataOption[{{$g_opt}}-{{$op}}][soluong_website]" id="soluong_option_{{$g_opt}}_{{$op}}" placeholder="Số lượng" value="{{$option['soluong_website']}}">
		                                							{{-- <div class="input-group-append">
		                                								<div class="input-group-text"><strong>Gram</strong></div>
		                                							</div> --}}
		                                						</div>
		                                                    ...
		                                                    @if(isset($config[$type]['giacu']) && $config[$type]['giacu'] == true)
		                                                        ...
		                                                        <div class="form-group col-md-6">
		                                							...
		                                							<div class="input-group">
		                                								<input type="text" class="form-control format-price gia_cu_op gia_cu_{{$g_opt}}_{{$op}}" name="dataOption[{{$g_opt}}-{{$op}}][giacu]" id="giacu_option_{{$g_opt}}_{{$op}}" placeholder="Giá cũ" value="{{$option['giacu']}}" data-position="{{$op}}" data-groupPos="{{$g_opt}}">
		                                								<div class="input-group-append">
		                                                    ...
		                                                    {{-- @if(isset($config[$type]['gia']) && $config[$type]['gia'] == true) --}}
		                                                        ...
		                                                        <div class="form-group col-md-6">
		                                							...
		                                							<div class="input-group">
		                                								<input type="text" class="form-control format-price gia_ban_op gia_ban_{{$g_opt}}_{{$op}}" name="dataOption[{{$g_opt}}-{{$op}}][gia]" id="gia_{{$g_opt}}_{{$op}}" placeholder="Giá bán" value="{{$option['gia']}}" data-position="{{$op}}" data-groupPos="{{$g_opt}}">
		                                								<div class="input-group-append">
		                                									<div class="input-group-text"><strong>VNĐ</strong></div>
		                                								</div>
		                                							</div>
		                                                    ...
		                                                    {{-- @if(isset($config[$type]['giamoi']) && $config[$type]['giamoi'] == true) --}}
		                                                        ...
		                                                        <div class="form-group col-md-6">
		                                							...
		                                							<div class="input-group">
		                                								<input type="text" class="form-control format-price gia_moi_{{$g_opt}}_{{$op}}" name="dataOption[{{$g_opt}}-{{$op}}][giamoi]" id="giamoi_{{$g_opt}}_{{$op}}" placeholder="Giá mới" value="{{$option['giamoi']}}">
		                                								<div class="input-group-append">
		                                									<div class="input-group-text"><strong>VNĐ</strong></div>
		                                								</div>
		                                							</div>
		                                                    ...
		                                                    @if(isset($config[$type]['giacu']) && $config[$type]['giacu'] == true)
		                                                        ...
		                                                        <div class="form-group col-md-6">
		                                							...
		                                							<div class="input-group">
		                                								<input type="text" class="form-control gia_km_op gia_km_{{$g_opt}}_{{$op}}" name="dataOption[{{$g_opt}}-{{$op}}][giakm]" id="giakm_option_{{$g_opt}}_{{$op}}" placeholder="Chiết khấu" value="{{$option['giakm']}}" maxlength="3" readonly data-position="{{$op}}" data-groupPos="{{$g_opt}}">
		                                								<div class="input-group-append">
		                                            ...
		                                            <div class="modal-footer">
		                                                <button type="button" class="btn btn-primary submit-check-option" data-position="{{$op}}" data-dismiss="modal" data-groupPos="{{$g_opt}}">Xác nhận</button>
		                                            </div>
		                                        </div>
		                                    </div>
		                                </div>
		        	...
		        	@if(isset($config[$type]['mau']) && $config[$type]['mau'] == true)
			        	<div class="form-group col-12" id="photo-upload-group-{{$g_opt}}">
		                    <label for="filer-gallery-{{$g_opt}}" class="mb-3 label-filer-gallery">Hình ảnh ({{ $config[$type]['gallery'][$type]['img_type_photo'] }})</label>
		                    <input type="file" name="files-{{$g_opt}}[]" id="filer-gallery-{{$g_opt}}" class="filer-gallery-group" data-idgroup="{{$g_opt}}" multiple="multiple">
		                    ...
		                    <input type="hidden" class="folder-filer" name="type" value="{{ $type }}_color">
		                    <input type="hidden" name="hash_color[]" value="{{ Str::random(8) }}" />
		                    <input type="hidden" name="id_photo" value="{{($row_product) ? $row_product['id'] : ''}}">
		                ...
		                @if(isset($gallery) && count($gallery) > 0)
		                    <div class="form-group form-group-gallery form-group-gallery-{{$g_opt}} col-12">
		                        ...
		                        <div class="jFiler-items my-jFiler-items jFiler-row">
		                            <ul class="jFiler-items-list jFiler-items-grid row scroll-bar" id="jFilerSortable-{{$g_opt}}">
...
<script>
	$('.btn-giachange').click(function(){
		...
		$.ajax({
			...
			data: {id:e_id,gia:e_val,typePrice:e_type,table:e_table,_token:$('input[name="_token"]').val()},
			success: function(response){				
				if(response.result=='success'){
					if(!e_present.hasClass('btn-giachange-success')){
						e_present.addClass('btn-giachange-success');
						setTimeout(function() {e_present.removeClass('btn-giachange-success')}, 2000);
					}
				}else{
					if(!e_present.hasClass('btn-giachange-error')){
						e_present.addClass('btn-giachange-error');
						setTimeout(function() {e_present.removeClass('btn-giachange-error')}, 2000);
					}
				}
			}
		});
	...
	@if(count($options_group)>0)
		@foreach($options_group as $g_opt=>$g_option)
			...
			$("#filer-gallery-{{$g_opt}}").attr({'data-hash':val_hash});
		@endforeach
		...
		$('.filer-gallery-group').each(function(){
			...
			$(this).filer({
		        ...
		        afterShow: function(){
		        	var jFilerItems = $(e_parent_group).find(".my-jFiler-items .jFiler-items-list li.jFiler-item");
		        ...
		        uploadFile: {
					...
					data: {data:$(this).data(),id_group:e_idgroup,model:$(e_parent_group).find("input[name='model']").val(),level:$(e_parent_group).find("input[name='level']").val(),type:$(e_parent_group).find("input[name='type']").val(),id:$(e_parent_group).find("input[name='id_photo']").val(),id_color:$(e_parent_group).find("input[name='id_color']").val(),time:$(e_parent_group).find('input[name="folder-time"]').val(), hash:$(e_parent_group).find("input[name='hash_color[]']").val(),_token:$('input[name="_token"]').val()},
					type: 'POST',
					...
					onComplete: function(){
						...
						{
							$(e_parent_group).find(".jFiler-items-list").first().find(".jFiler-item").remove();
		...
		function Color_refreshFilerIfEmpty(e_idgroup,element_parent_group){
			...
			$.ajax({
				...
				async: false,
				data: {idmuc:idmuc,id_color:id_color,com:com,kind:actfiler,type:type,colfiler:colfiler,time:time,cmd:cmd,hash:hash,_token:_token},
				success: function(result){
	                ...
	                $tmp = '<div class="form-group form-group-gallery form-group-gallery-'+e_idgroup+' col-12">';
	                if(time==1){
					...
					+'<ul class="jFiler-items-list jFiler-items-grid row scroll-bar" id="jFilerSortable-'+e_idgroup+'">'
					+result
					+'</ul></div></div>';
				...
				sortable = new Sortable.create(document.getElementById('jFilerSortable'),{
					...
					onEnd: function(){
						...
						$.ajax({
							...
							dataType: 'json',
							async: false,
							data: {idmuc:idmuc,id_color:id_color,listid:listid,com:com,kind:actfiler,type:type,colfiler:colfiler,cmd:'updateNumb',hash:hash,_token:_token},
							success: function(result){
								var arrid = result.id;
								var arrnumb = result.numb;
								for(var i=0;i<arrid.length;i++) $(element_parent_group).find('.my-jFiler-item-'+arrid[i]).find("input[type=number]").val(arrnumb[i]);
							}
						})
					},
				});
