			...
			<div class="manage-form-right">
						...
						@foreach($thongbao as $k=>$v)
							...
							<div class="sidebar-tab-inform-item sidebar-tab-inform-item-{{$v['id']}} {{($v['daxem']==1) ? 'sidebar-tab-inform-item-hasview': ''}}">
								<div class="sidebar-tab-inform-topitem">
									<p class="sidebar-tab-inform-toptitle"><span class="mr-2"></span>{{__('Thông báo')}}</p>
								...
								<div class="sidebar-tab-inform-botitem">
									...
									@if(isset($info['type']) && $info['type']=='comment')	
										...
										@endphp							
										<a href="{{$post['tenkhongdau'.$lang]}}-{{$post['id']}}?comment={{$info['id_comment']}}" target="_blank" class="sidebar-tab-inform-view"><i class="fal fa-hand-point-right mr-1"></i>{{__('Xem chi tiết')}}</a>
									@endif
				...
				<div class="sidebar-tab-showtool">
					...
					<div class="sidebar-tab-showtool-list">
						...
						<p class="sidebar-btn-status sidebar-btn-status-remove" data-status="xoa" data-id=""><i class="fas fa-trash mr-2"></i> {{__('Xóa thông báo')}}</p>
						<p class="sidebar-btn-cancel"><i class="fas fa-times mr-2"></i> {{__('Hủy')}}</p>
					</div>
				</div>
			</div>
		</form>
