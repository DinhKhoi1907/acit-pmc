        ...
        <div class="relative flex content-page-layout">
            <div class="w-full lg:w-[877px]">
                ...
                @if(isset($posts) && count($posts) > 0)
                    <div class="mt-10">
                        <div class="text-2xl md:text-[28px] font-bold text-black relative mb-12 border-0 border-b border-solid border-cmain2 border-opacity-30 pb-2 flex items-center justify-between">
                            <p class="uppercase">{{__('Forex Brokers')}}<span class="absolute w-[80px] border-0 border-t-[4px] border-cmain border-solid -bottom-[8px] md:-bottom-[10px] left-0 pb-2 transition-all duration-500 group-hover:border-black"></span></p>
                        </div>
                        <div class="flex flex-wrap gap-[12px] sm:gap-[30px]">
                            @foreach($posts as $k=>$v)
                                <div class="w-[calc(100%/2-6px)] sm:w-[calc(100%/2-22.5px)] md:w-[calc(100%/3-22.5px)] text-center bg-gray-100 py-10 px-5 sahdow-shadow1 rounded-md relative group">
                                    <a href="{{($v['linklienket']) ? $v['linklienket'] : $v['tenkhongdau'.$lang]}}" title="{{$v['ten'.$lang]}}" class="absolute top-0 left-0 z-50 w-full h-full transition-all duration-500 border border-solid rounded-md border-cmain2 border-opacity-20 group-hover:border-opacity-100"></a>
                                    <a href="{{($v['linklienket']) ? $v['linklienket'] : $v['tenkhongdau'.$lang]}}" title="{{$v['ten'.$lang]}}" class="himg"><img class="inline-block transition-all duration-500 group-hover:scale-90" src="{{ Thumb::Crop(UPLOAD_POST, $v['photo'], 125, 125, 1) }}" alt="" width="125" height="125"></a>
