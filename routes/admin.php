...
Route::group(['name'=>'admin.','prefix'=>app('settingOptions')['linkadmin'], 'middleware'=>['auth:admin','AuthAdmin']], function(){
	/*
    |--------------------------------------------------------------------------
    | Route clear cache
    |--------------------------------------------------------------------------
    ...
    */
	...
	Route::group(['name'=>'inform.', 'prefix'=>'inform'],function(){
		Route::get('/expireToken', 'InformController@ExpireToken')->name('admin.inform.expireToken');
	});


	/*
    |--------------------------------------------------------------------------
    | Route xử lý table mysql
    |--------------------------------------------------------------------------
    */
	Route::group(['name'=>'table.', 'prefix'=>'table'],function(){
		Route::get('/show/{tablename?}', 'TableController@Show')->name('admin.table.show');
		Route::get('/edit/{tablename?}/{tablecolumn?}', 'TableController@Edit')->name('admin.table.edit');
    ...
    */
	...
	Route::get('/ajax-load-product', 'AjaxController@LoadProduct')->name('admin.ajax.loadProduct');
	Route::get('/ajax-load-property', 'AjaxController@LoadProperty')->name('admin.ajax.loadProperty');
    ...
    */
	...
	/*
    |--------------------------------------------------------------------------
    | Route trang user admin
    |--------------------------------------------------------------------------
    */
	Route::group(['name'=>'users.', 'prefix'=>'users', 'middleware'=>'CheckModel:Users'],function(){
		//Route::get('/editchange', 'Admin\UsersController@EditChangePassword')->name('admin.users.editchange'); // show trang thay đổi mật khẩu
		//Route::post('/change', 'Admin\UsersController@ChangePass')->name('admin.users.change'); // cập nhật thay đổi mật khẩu

		//### Chỉ role:3 mới có quyền những quyền dưới đây
		Route::get('/show/{query?}', 'UsersController@Show')->name('admin.users.show'); // show trang ds tài khoản admin
		Route::get('/edit/{id?}', 'UsersController@Edit')->name('admin.users.edit'); // show trang thêm tài khoản admin
		Route::post('/save', 'UsersController@Save')->name('admin.users.save'); // lưu tài khoản admin mới
		Route::get('/delete/{id}', 'UsersController@Delete')->name('admin.users.delete'); // xóa (chỉ role:3 mới có quyền này)
		Route::get('/deleteall/{listid?}', 'UsersController@DeleteAll')->name('admin.users.deleteAll'); // xóa all
	});
    ...
    */
	Route::group(['name'=>'product.', 'prefix'=>'product', 'middleware'=>'CheckModel:Product'], function(){		
		...
		Route::get('/edit/{category}/{type}/{id?}', 'ProductController@Edit')->name('admin.product.edit'); // thêm mới hoặc cập nhật
    ...
    */
    ...
    |--------------------------------------------------------------------------
    */
	Route::group(['name'=>'binhluan.', 'prefix'=>'binhluan', 'middleware'=>'CheckModel:BinhLuan'], function(){
		Route::get('/show/{category}/{query?}', 'BinhLuanController@Show')->name('admin.binhluan.show'); //load ds

		Route::get('/edit/{category}/{id?}', 'BinhLuanController@Edit')->name('admin.binhluan.edit'); // thêm mới hoặc cập nhật

		Route::get('/delete/{category}/{id}', 'BinhLuanController@Delete')->name('admin.binhluan.delete'); // xóa

		Route::get('/deleteall/{category}/{listid?}', 'BinhLuanController@DeleteAll')->name('admin.binhluan.deleteAll'); // xóa all

		Route::get('/report/{category}/{id}', 'BinhLuanController@Report')->name('admin.binhluan.report'); // xóa

		Route::post('/save/{category}', 'BinhLuanController@Save')->name('admin.binhluan.save'); // lưu
	});
    ...
    */
	Route::group(['name'=>'post.', 'prefix'=>'post', 'middleware'=>'CheckModel:Post'], function(){
		...
		Route::get('/edit/{category}/{type}/{id?}', 'PostController@Edit')->name('admin.post.edit');
    ...
    */
	Route::group(['name'=>'order.', 'prefix'=>'order', 'middleware'=>'CheckModel:Order'], function(){
		...
		Route::match(['get', 'post'], '/import/{category}/{query?}', 'OrderController@ImportBillGet')->name('admin.order.import');
	});
