...

{
    ...
    private static $albumRepo, $brandRepo, $colorRepo, $galleryRepo, $photoRepo, $postRepo, $productOptRepo, $productRepo, $sizeRepo, $staticRepo, $tagRepo, $newsletterRepo, $contactRepo;
    ...
    */
    ...
    public static function get_order_total($id_user=0,$token_member_cart='',$coupon=null){
        ...
        //### lấy thông tin voucher nếu tồn tại
        $mucgia = 0;
        $ids_product = null;
        if($coupon!=null){
            $mucgia = $coupon['mucgiam'];
            $ids_product = ($coupon['ids_product']!='') ? json_decode($coupon['ids_product'],true) : null;
        }
        ...
        if($row_exist){
            ...
            for($i=0;$i<count($row_exist);$i++){
    			...
    			$proinfo = self::get_product_info($pid,$size,$mau,$mucgia*$q,$ids_product);
    ...
    public static function get_order_coupon($id_user=0,$token_member_cart='',$coupon=null){
        ...
        if($row_exist){
            ...
            for($i=0;$i<count($row_exist);$i++){
                ...
                if($proinfo['giamoi_coupon']) $price = $proinfo['giamoi_coupon'];

                else $price = $proinfo['gia_coupon'];
    ...
    */
    ...
    public static function get_product_info($pid=0,$size=0,$mau=0,$mucgiam=0,$ids_product=null){
		...
		if($pid){
            ...
            $row['gia_coupon'] = $row['giacu_coupon'] = $row['giamoi_coupon'] = 0;
			...
			if($row1){
                ...
                //### thiết lập giá coupon
                if($mucgiam>0 && $ids_product!=null && in_array($pid, $ids_product)){
                    $giasaugiam = ($row['gia']*$mucgiam)/100;
                    $row['gia_coupon'] = $giasaugiam;
                    $row['gia'] = $row['gia'] - $giasaugiam;
                }

                //### thiết lập giá coupon
                if($mucgiam>0 && $ids_product!=null && in_array($pid, $ids_product)){
                    $giasaugiam = ($row['giacu']*$mucgiam)/100;
                    $row['giacu_coupon'] = $giasaugiam;
                    $row['giacu'] = $row['giacu'] - $giasaugiam; 
                }

                //### thiết lập giá coupon
                if($mucgiam>0 && $ids_product!=null && in_array($pid, $ids_product)){
                    $giasaugiam = ($row['giamoi']*$mucgiam)/100;
                    $row['giamoi_coupon'] = $giasaugiam;
                    $row['giamoi'] = $row['giamoi'] - $giasaugiam; 
                }
