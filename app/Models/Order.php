...
class Order extends Model
{
    ...
    */
    public function HasOrderDetailAll() {
        return $this->hasMany('App\Models\OrderDetail','id_order')->with(['BelongProduct']);
    }
    ...
    */
    ...
    |--------------------------------------------------------------------------
    */
    public function GetItems($params=null,$orderby=null){
        $run_sql = $this->with('HasOrderDetailAll');
        if($params){
            foreach($params as $k=>$v){
                $run_sql=$run_sql->where($k, $v);
    ...
    public function GetAllItems($params){
        ...
        if($params){
            ...
            foreach($params as $k=>$v){
                ...
                if($k=='keyword'){
                    //$run_sql=$run_sql->like('hoten', $v)->orWhere('dienthoai', 'like' , '%'.$v.'%'); //->orWhere('dienthoai', 'like' , '%'.$v.'%')
                    $run_sql=$run_sql->whereRaw("(hoten like '%".$v."%' or dienthoai like '%".$v."%')");
                }
