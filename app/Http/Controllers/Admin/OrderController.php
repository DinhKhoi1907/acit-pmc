...
class OrderController extends Controller
{
    ...
    */
    public function Show(Request $request){
        ...
        if(isset($itemCountOrder) && (int)$itemCountOrder['giamin']>0) $minTotal = $itemCountOrder['giamin'];
		else $minTotal = 0;

		if(isset($itemCountOrder) && (int)$itemCountOrder['giamax']>0) $maxTotal = $itemCountOrder['giamax'];
		else $maxTotal = 0;
        ...
        ->labels(['('.((isset($itemCountOrder)) ? $itemCountOrder['allMoidat'] : 0).') Mới đặt', '('.((isset($itemCountOrder)) ? $itemCountOrder['allDaxacnhan'] : 0).') Đã xác nhận', '('.((isset($itemCountOrder)) ? $itemCountOrder['allDanggiao'] : 0).') Đang giao', '('.((isset($itemCountOrder)) ? $itemCountOrder['allDangchuyenhoan'] : 0).') Đang chuyển hoàn', '('.((isset($itemCountOrder)) ? $itemCountOrder['allDachuyenhoan'] : 0).') Đã chuyển hoàn', '('.((isset($itemCountOrder)) ? $itemCountOrder['allDagiao'] : 0).') Đã giao', '('.((isset($itemCountOrder)) ? $itemCountOrder['allDahuy'] : 0).') Đã hủy'])
        ->datasets([
            [
                'backgroundColor' => ['#007bff', '#17a2b8', '#ffc107', '#dc3545', '#28a745', '#26b99a', '#6c757d'],
                'data' => [(isset($itemCountOrder)) ? $itemCountOrder['allMoidat'] : 0, (isset($itemCountOrder)) ? $itemCountOrder['allDaxacnhan'] : 0, (isset($itemCountOrder)) ? $itemCountOrder['allDanggiao'] : 0, (isset($itemCountOrder)) ? $itemCountOrder['allDangchuyenhoan'] : 0, (isset($itemCountOrder)) ? $itemCountOrder['allDachuyenhoan'] : 0, (isset($itemCountOrder)) ? $itemCountOrder['allDagiao'] : 0, (isset($itemCountOrder)) ? $itemCountOrder['allDahuy'] : 0]
            ]
        ])
        ->options([]);
            ...
            if($row_order['tinhtrang']!='5' && $row_order['tinhtrang']!='7'){
                ...
                foreach($row_details as $k=>$v){
                    if($v['table_name']=='product_option'){
                        ...
                        if($result_product['phienbanmau']==1){
                            $row_parent = $this->productRepo->GetOneItem($result_product['id_product']);
                            $data['soluong_website'] = $row_parent['soluong_website'] + $v['soluong'];
                            $result_end = $this->productRepo->SaveItem($data,$row_parent['id']);
                        }
                    }else{
    ...
    */
    public function Save(Request $request){
        ...
        //###Tạo thông báo
        $check_tb = false;
        if($row_order['tinhtrang']!=$data['tinhtrang']){
            $check_tb = true;
        }

        if($row['id_user']>0 && $check_tb){
            $text_tb = '';

            switch($row['tinhtrang']){
                case 2: //Đã xác nhận
                    $text_tb = 'Hệ thống đã xác nhận đơn hàng của bạn. Hệ thống sẽ thông báo khi đơn hàng được chuyển qua giai đoạn tiếp theo';
                    break;
                case 3: //Đang giao hàng
                    $text_tb = 'Đơn hàng đang trong quá trình vận chuyển tới quý khách.';
                    break;
                case 4: //Đã giao
                    $text_tb = 'Đơn hàng đã được giao tới quý khách';
                    break;
                case 5: //Đã hủy
                    $text_tb = 'Đơn hàng bị hủy vì 1 vài lý do.';
                    break;
                case 6: //Đang chuyển hoàn
                    $text_tb = 'Đơn hàng đang trong quá trình chuyển hoàn.';
                    break;
                case 7: //Đã chuyển hoàn
                    $text_tb = 'Đơn hàng đã được chuyển hoàn thành công.';
                    break;
            }


            if(config('config_all.notify')==true ){
                $data_thongbao['tieudevi'] = 'Cập nhật trạng thái đơn hàng #'.$row['madonhang'];
                $data_thongbao['tieudeen'] = 'Update order status #'.$row['madonhang'];
                $data_thongbao['sub_tieudevi'] = 'Cập nhật trạng thái đơn hàng #'.$row['madonhang'];
                $data_thongbao['sub_tieudeen'] = 'Update order status #'.$row['madonhang'];
                $data_thongbao['noidung'] = $text_tb;
    ...
    private function getDataViettelPost($result){
        if($result && !$result['is_vandon']){
            ...
            $data = array(
                ...
                "PRODUCT_TYPE" => "HH",
                "ORDER_PAYMENT" => 3, //### thu hộ tiền hàng
