...
class PhotoController extends Controller
{
    ...
    */
    public function Add(Request $request){
        ...
        $type = $request->type;
        $id = $request->id;
        $numberPhoto = (isset($this->config[$type]['number']))? $this->config[$type]['number'] : 1;
    ...
    public function Save(Request $request){
        ...
        if($id)
        {
            ...
            if($request->hasFile('video'))
            {
                ...
                if($newimage){ $data['video'] = Helper::UploadVideoToFolder($newimage, $oldimage, $this->folder_save); }
            }
            ...
            $data['hienthi_video'] = (isset($data['hienthi_video'])) ? 1 : 0;
            ...
            if($numberPhoto && $dataMultiTemp)
            {
                for($i=0;$i<count($dataMultiTemp);$i++)
                {
                    ...
                    $dataMulti['hienthi_video'] = (isset($dataMultiTemp[$i]['hienthi_video'])) ? 1 : 0;
                    $dataMulti['type'] = $type;
                    ...
                    if(isset($this->config[$type]['images']) && $this->config[$type]['images'] == true)
                    {
                        ...
                        {
                            $oldimage = '';
                            $newimage = $request->file('video'.$i);

                            if($newimage){
                                $dataMulti['video'] = Helper::UploadVideoToFolder($newimage, $oldimage, $this->folder_save);
                            }
                        }
