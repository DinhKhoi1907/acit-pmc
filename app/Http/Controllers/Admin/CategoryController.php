...
class CategoryController extends Controller
{
    ...
    */
    public function Show(Request $request){        
        ...
        //### Code xử lý...
        $itemShow = $this->categoryRepo->GetAllItems($type,$params,$this->relations,$this->pagination);
    ...
    */
    public function Edit(Request $request){
        ...
        if(isset($rowItem)){            
            $id_category = $rowItem['id_parent'];
            $category_select = $this->categoryRepo->Getitem(['id'=>$id_category]);
            $category_list_same = ($id_category > 0) ? $this->categoryRepo->GetAllItemsExceptId($category_select['type'],['level'=>$category_select['level'], 'id'=>$id_category]) : null;

            $category = array(
                'id' => $rowItem['id'],
                'id_parent' => $rowItem['id_parent'],
                'tenvi' => $rowItem['tenvi'],
                'tenvi_parent' => (isset($rowItem['category_parent']['tenvi'])) ? $rowItem['category_parent']['tenvi'] : ''
            );            
        }
        ...
        //### lấy properties
        $thuoctinhs = $this->categoryRepo->GetAllItems('thuoctinh', ['hienthi'=>1]);
        $thuoctinh_selected = $this->categoryPropertyRepo->GetAllItemsByParamsPluck('id_property',['id_category'=>$id]);
        ...
        $response = array(
            ...
            'category_list_same' => (isset($category_list_same)) ? $category_list_same : null,
            'ids_parent' => (isset($rowItem)) ?  $rowItem['ids_parent'] : '',
            'prefix' => $prefix,
            'thuoctinhs' => $thuoctinhs,
            'thuoctinh_selected' => $thuoctinh_selected
        );
    ...
    */
    public function Save(Request $request){
        ...
        if($data){
            ...
            //### lưu thuộc tính liên quan dm sản phẩm
            if($row){
                $idCategory = $row['id'];
                ...
                if($dataProperty){
                    ...
                    $this->categoryPropertyRepo->Query()->where('id_category',$idCategory)->delete();

                    //### lưu các rows mới
                    if($row['level']==0){
                        foreach($dataProperty as $k=>$v){
                            $this->categoryPropertyRepo->SaveItem(['id_category'=>$idCategory, 'id_property'=>$v]);
                        }
                    }                    
                }
            }
