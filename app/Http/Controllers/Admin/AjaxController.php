...
{	
    ...
    */
	public function LoadProduct(Request $request){
		...
		$ids_product_select = array();
		if($id){
			$coupon = $this->couponRepo->GetOneItem($id);
			$ids_product_select = ($coupon && $coupon['ids_product']!='') ? json_decode($coupon['ids_product'], true) : array();
		}

		//### Lấy ds sản phẩm
		$products = $this->productRepo->GetAllItems('product',['hienthi'=>1]);
		$response = array(
			'products' => $products,
			'ids_product_select' => $ids_product_select
		);
		return view('admin.layouts.show_products')->with($response);
	}
    ...
    */
	public function LoadProperty(Request $request){
		...
		$thuoctinh_of_category = $this->categoryPropertyRepo->GetAllItemsByParamsPluck('id_property',['id_category'=>$id]);
		$thuoctinhs = $this->categoryRepo->GetAllItemByIds($thuoctinh_of_category);
		$values_of_thuoctinh = $this->productPropertyRepo->GetAllItemsByParamsPluck('id_value',['id_product'=>$id_product]);
		
		$response = array(
			'thuoctinh_of_category' => $thuoctinh_of_category,
			'thuoctinhs' => $thuoctinhs,
			'values_of_thuoctinh' => $values_of_thuoctinh
		);

		return view('admin.layouts.show_property')->with($response);
	}
    ...
    */
	public function LoadSelectCategory(Request $request){
		...
		if($level>0){
			if($list_ids){
				if($type=='tintuc' && Auth::guard('admin')->user()->role!=3){
		            $row_category = array();
		            //### láy quyền của subadmin
		            $ids = DB::table('role_has_permissions')->select('permission_id')->where('role_id',Auth::guard('admin')->user()->id_nhomquyen)->get()->pluck('permission_id');     
		            if($ids){
		                $ids_tmp = array();
		                $ids = $ids->toArray();
		                $permissions = DB::table('permissions')->whereIn('id',$ids)->get()->pluck('name');
		                if($permissions){
		                  foreach($permissions as $k=>$v){
		                    $arr_value = explode('_',$v);
		                    array_push($ids_tmp, $arr_value[count($arr_value)-1]);
		                  }  
		                }

		                if($ids_tmp){		                	
		                    $categories = $this->categoryRepo->GetAllItemByIdsParam($ids_tmp, ['level'=>$level-1]);
		                }
		            }
    ...
    */
    public function AddCart(Request $request){    	
    	...
    	if($option_new!=''){
    		...
    		$size_new=$sm[0]; $mau_new=$sm[1];
    	}else{
    		$size_new=0; $mau_new=0;
    	}
        			...
        			$data['html_text'].='<div class="info-procart">';
        				...
        				$data['html_text'].='<div class="properties-procart">';
                            if($mau) {
        						...
        						if($maudetail){$data['html_text'].='<p>Màu: <strong>'.$maudetail['tenvi'].'</strong></p>';}
        					}
        					if($size) {
                                $sizedetail=CartHelper::get_size_info($size);
        						if($sizedetail){$data['html_text'].='<p>Size: <strong>'.$sizedetail['tenvi'].'</strong></p>';}
        					}
        				$data['html_text'].='</div>';
    ...
    */
    public function UploadImage(Request $request){
        ...
        }else if(isset($id_group) && $request->hasFile('files-'.$id_group)){
    ...
    */
    ...
    public function Properties(Request $request){
        ...
        if(isset($id_product)){        	//&& $id_product>0
            ...
            $parent_masp = (isset($row_product['masp']))?$row_product['masp']:'';
                ...
                foreach($mau_group as $m=>$mau){
                    ...
                    foreach($size_group as $s=>$size){
                        ...
                        if($row_pro_option){
							...
							$options_tmp[$m][$s]['soluong_website'] = '';                    
                        }
                        array_push($options, $options_tmp[$m][$s]);
                    }
                }         
            }else if(isset($mau_group)){
                //tạo mảng product con
                foreach($mau_group as $m=>$mau){
                    $row_color = $model_color->GetOneItem($mau, $this->relations);
                    ...
                    if($row_pro_option){
						...
						$options_tmp[$m]['soluong_website'] = '';
                    }
                    array_push($options, $options_tmp[$m]);
                }
            }else if(isset($size_group)){
                //tạo mảng product con
                foreach($size_group as $s=>$size){
                    $row_size = $model_size->GetOneItem($size, $this->relations);
