...
class CartController extends Controller
{
    ...
    */
    public function ShowCart($response, Request $request){
        ...
        //$tientragopdinhky = 5000000;
        //$listTragop = $this->GetInstallment($tientragopdinhky);

        //## kiểm tra đơn hàng có trả góp ?       
        $isDebit = (isset($request->debit)) ? $request->debit : false;
        ...
        if($field){OpenGraph::addProperty('type', 'article');}else{OpenGraph::addProperty('type', 'object');}
    ...
    */
    public function OrderCart(Request $request){
        ...
        $month = (isset($data_post['month']) && $data_post['month'] > 0) ? $data_post['month'] : 0;
        $bankCode = (isset($data_post['bankCode']) && $data_post['bankCode'] !='') ? $data_post['bankCode'] : '';
        $paymentMethod = (isset($data_post['paymentMethod']) && $data_post['paymentMethod'] !='') ? $data_post['paymentMethod'] : '';


        //### get hình thức thanh toán
        $row_httt = $this->postRepo->GetItem(['type'=>'hinh-thuc-thanh-toan', 'id'=>$httt]);
        $htttText = (isset($row_httt)) ? $row_httt['ten'.$this->lang] : '';
        $pay_status=0;
        ...
        if($id_login>0){
            ...
            $this->model = $this->model->where('id_user',$id_login)->where('token_member_cart',$token_member_cart);
        ...
        //### Test thanh toán
        //$row_order = $model_order->GetOneItem(684);
        ...
        $data_donhang['dienthoai'] = str_replace(' ', '',$dienthoai);
        ...
        $data_donhang['month'] = $month;
        $data_donhang['bankCode'] = $bankCode;
        $data_donhang['paymentMethod'] = $paymentMethod;
        ...
        if($request->hasFile('file')){
            ...
            if($newimage){ $data_donhang['photo'] = Helper::UploadImageToFolder($newimage, $oldimage, $folder); }
        }
        ...
        if(isset($row_order->id)){
            ...
            for($i=0;$i<count($row_cart);$i++){
                ...
                $r_color = CartHelper::get_mau_info($row_cart[$i]['mau']);
                $r_size = CartHelper::get_size_info($row_cart[$i]['size']);
                $color = (isset($r_color)) ? $r_color['ten'.$lang] : 0;
                $size = (isset($r_size)) ? $r_size['ten'.$lang] : 0;
                ...
                if((config('config_all.order.soluong') || config('lazada.active')) && $row_orderDetail){
                    if($table_name=='product_option'){
                        ...
                        //### nếu sản phẩm là phiên bản
                        if($row['phienbanmau']==1){
                            $row_parent = $this->productRepo->GetOneItem($row['id_product']);
                            $data['soluong_website'] = $row_parent['soluong_website'] - $q;
                            $result_product = $this->productRepo->SaveItem($data,$row_parent['id']);
                        }
                    }else{
            ...
            if($row_order){
                ...
                if($email != '' && (!$this->is_online || $payment_method=='COD') ){
                    $message = array();
                    $message['tieude'] = thongtindathang." ".$madonhang;
            ...
            if (($payment_method == "alepay" || $payment_method == "alepay-domestic") && $row_order) {
                //### gửi mail tới người đặt hàng
                Mail::to($email)
                ->cc($this->setting_opt['email'])
                ->send(new MailNotify($message, null, 'order'));
                ### gửi mail tới quản trị viên
                // Mail::to($this->setting_opt['email'])->send(new MailNotify($message, null, 'order'));
                // Mail::to('huuduong9@gmail.com')->send(new MailNotify($message,null,'order'));
                //### xử lý payment
    ...
    public function OrderCartAgain(Request $request){
        $this->initialization($request);
        ...
        if($request->id){
            ...
            $model_order = new Order();
            $id = $request->id;
            $row_order = $model_order->GetItem(['id'=>$id,'id_user'=>$user['id']]);

            if(!$row_order){
                return redirect()->route('cart.inform',['status'=>'error']);
            }

            //### cho cho phép thanh toán lại với đơn hàng không thành công
            if($row_order['status_payments']==3){
                $message = array();
                ...
                if (($row_order['payment_method'] == "alepay" || $row_order['payment_method'] == "alepay-domestic") && $row_order) {
                    //### gửi mail tới người đặt hàng
                    Mail::to($row_order['email'])
                    ->cc($this->setting_opt['email'])
                    ->send(new MailNotify($message, null, 'order'));
                    ### gửi mail tới quản trị viên
                    // Mail::to($this->setting_opt['email'])->send(new MailNotify($message, null, 'order'));
                    // Mail::to('huuduong9@gmail.com')->send(new MailNotify($message,null,'order'));
                    //### xử lý payment
                    $request['option_payment'] = $row_order['payment_method'];
                    $this->CallPayment($row_order->toArray(), $request);
                }else if ($row_order['payment_method'] == "momo" && $row_order) {
                    //### gửi mail tới người đặt hàng
                    Mail::to($row_order['email'])
                    ->cc($this->setting_opt['email'])
                    ->send(new MailNotify($message, null, 'order'));
    ...
    private function CallPayment($detail_order, $request){
        if ($detail_order) {
            ...
            if ($config_nganluong_active) {
                ...
                $data['checkoutType'] = ($payment_method=='alepay-domestic') ? 2 : 4; // Thanh toán trả góp
                ...
                if($payment_method=='alepay-domestic'){
                    $data['month'] = $detail_order['month'];
                    $data['bankCode'] = $detail_order['bankCode'];
                    $data['paymentMethod'] = $detail_order['paymentMethod'];
                }
                ...
                //     }
                // }
                $data['allowDomestic'] = ($payment_method=='alepay-domestic') ? false : true;
                $data['installment'] = ($payment_method=='alepay-domestic') ? true : false;
                ...
                if ($payment_method !='' && $buyer_email !="" && $buyer_mobile !="" && $buyer_fullname !="" && filter_var($buyer_email, FILTER_VALIDATE_EMAIL)) {                    
                    ...
                    if ($result->code != '') {
                        if ($result->code == '000') {
                            ...
                            //### Lỗi: chưa nhập đủ thông tin ==> thanh toán không thành công
                            $order = Order::find($detail_order['id']); 
                            $order->status_payments = 3;
                            $order->is_online = 1;
                            $order->save();
    ...
    */
    public function OrderInform(Request $request){
        ...
        switch ($status) {
            case 'success':
                $text = "Cảm ơn quý khách đã mua hàng tại website. Đơn hàng của quý khách đang được hệ thống xử lý. Mọi thông tin về đơn hàng sẽ được gửi tới email của bạn, vui lòng kiểm tra email để biết thêm chi tiết.";                
                break;
            case 'error':
                $text = "Xảy ra lỗi !";                
                break;
