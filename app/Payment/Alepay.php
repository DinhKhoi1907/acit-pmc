...
class Alepay
{
    ...
    public function sendOrderV3($data)
    {
        ...
        Log::channel('payment')->info('Alepay sendOrderV3', $data);
        // dd($data_string);
        $url = $this->baseURL[$this->env] . $this->URI['requestPayment'];
        // print_r($url);
    ...
    public function getTransactionInfoV3($transactionCode)
    {
        ...
        Log::channel('payment')->info('Alepay send getTransactionInfoV3', $dataInfo);
        ...
        $result = curl_exec($ch);
        Log::channel('payment')->info('Alepay info getTransactionInfoV3', json_decode($result, true));
        return json_decode($result);
    }


    public function getInstallmentInfo($amount=0){
        $data['tokenKey'] = $this->apiKey;
        ...
        $dataInfo['currencyCode'] = $data['currencyCode'];

        $data_string = json_encode($dataInfo);
        Log::channel('payment')->info('Alepay getInstallmentInfo', $dataInfo);
